...
```{r}
ames10_holdout <- read.csv('AmesTest10.csv', stringsAsFactors = TRUE)
ames10_holdout <- na.omit(ames10_holdout)
ames10_holdout <- ames10_holdout[-1] 
ames10_holdout$age_built <- 2010 - ames10_holdout$YearBuilt
ames10_holdout$age_remodel <- 2010 - ames10_holdout$YearRemodel
ames10_holdout$Quality <- factor(ames10_holdout$Quality)
ames10_holdout$Condition <- factor(ames10_holdout$Condition)
```


When trying to predict on the test dataset we were getting errors because the test set contained 0 for BasementHt and BasementC.
We changed these to "None" becuase we assumed that is what they represent.
```{r}
ames10_holdout$BasementHt <- as.character(ames10_holdout$BasementHt) 
ames10_holdout$BasementHt[ames10_holdout$BasementHt == "0"] <- "None"
unique(ames10_holdout$BasementHt)
ames10_holdout$BasementC <- as.character(ames10_holdout$BasementC) 
ames10_holdout$BasementC[ames10_holdout$BasementC == "0"] <- "None"
unique(ames10_holdout$BasementC)
```

Some of our transformations were applied to the training dataset. So to make sure our predictions are properly implemented we must transform the test data first.

```{r}
ames10_holdout_adjusted <- ames10_holdout %>%
  mutate(Price = log(Price),
         LotArea = log(LotArea))
```
...
```{r}
predictions <- predict(simple_model, newdata = ames10_holdout_adjusted)
head(predictions)
```
...
house_ex <- house_ex[-1] 
house_ex$age_built <- 2010 - house_ex$YearBuilt
house_ex$age_remodel <- 2010 - house_ex$YearRemodel
house_ex$Quality <- factor(house_ex$Quality)
house_ex$Condition <- factor(house_ex$Condition)
house_ex$LotArea <- log(house_ex$LotArea)
house_ex
```
```{r}
predict.lm(simple_model, house_ex, interval = 'confidence', level = .95)
exp(predict.lm(simple_model, house_ex, interval = 'confidence', level = .95))
```
We predict but this gives us a value of log(price) so we can take exp() to make it in thousands of dollars.
We can say with 95% confidence that the mean value of homes with above characteristics falls between 228 and 276 thousand dollars.
