...
```{r}
#full_wfactor2 <- lm(Price ~ . + .^2, data=NewAmesTrain_wfactor)

```

```{r}
#MSE2 <- (summary(full_wfactor2)$sigma)^2
#AmesModNone2 <- na.omit(AmesModNone2)

#AmesModNone2 <- lm(Price ~ 1, data = NewAmesTrain_wfactor)
#simple_model2 <- step(AmesModNone2, scale = MSE2, scope = list(upper = full_wfactor2), trace = FALSE)
...
DoublenewNewAmesTrain <- DoublenewNewAmesTrain %>%
...
simple_model2 <- step(AmesModNone2, scale = MSE2, scope = list(upper = first_model), trace = FALSE)
summary(simple_model2)
